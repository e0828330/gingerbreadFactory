...
public class JMSMonitoringSender {
	...
	public JMSMonitoringSender(Context ctx, int factoryID) throws JMSException, NamingException {

		queueConnectionFactory = (QueueConnectionFactory) ctx.lookup(this.FACTORY);
		monitoring_queue = (Queue) ctx.lookup(this.MONITORING_QUEUE + factoryID);
